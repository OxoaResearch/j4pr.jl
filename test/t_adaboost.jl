...
function t_adaboost()
	...
	# M1
 	Base.Test.@test try
		...
		f_exec=(m,x)->j4pr.targets(indmax, j4pr.kNNClassifier.kNN_exec(m,j4pr.getobs(x),[1,2,3])) # [1,2,3] are the unique integer labels, sorted
		
		# Train 
		ae=j4pr.AdaBoost.adaboost_train(+tr, ytr, L, x->j4pr.kNNClassifier.kNN_train(x[1],x[2],3,smooth=:ml), f_exec, j4pr.AdaBoost.AdaBoostM1())
	...
	# M2	
 	Base.Test.@test try
		...
		f_exec=(m,x)->j4pr.kNNClassifier.kNN_exec(m,j4pr.getobs(x),[1,2,3])
		
		# Train
		ae=j4pr.AdaBoost.adaboost_train(+tr, ytr, L, x->j4pr.kNNClassifier.kNN_train(x[1],x[2],3,smooth=:ml), f_exec, j4pr.AdaBoost.AdaBoostM2())
	...
	A = j4pr.DataGenerator.iris() 				# load data
	tr,ts = j4pr.splitobs(j4pr.shuffleobs(A), 0.5)		# shuffle and split data
	w=j4pr.knn(5,smooth=:ml) 				# define classifier: 5-NN classifier, maximum likelihood posterior smoothing
	L=10	
